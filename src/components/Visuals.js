...
function Visuals() {
   ...
   useEffect(() => {
      if(!myState.play){
         ...
         document.getElementById('play-btn').style.backgroundColor = 'rgb(2 104 119)';
         document.getElementById('change-btn').disabled = false;
         document.getElementById('change-btn').style.backgroundColor = 'rgb(2 104 119)';
      }
   },[myState.play]);
            ...
            <div className="visual__items" style={{width:`${myState.values.length*11}px`}}>
               {
                  myState.values.map((item) => {
                     ...
                     return <div className="visual__item" key={item[1]} id={item[1]} style={{transition:`${speed/1000}s linear all`, transform:`translateX(${item[1]*14}px)`}}>
                              <h4>{item[0]}</h4>
                              <div className="visual" style={{height:`${item[0]*3}px`, backgroundColor:color, width:'8px'}}></div>
                           </div>
                  })
               }   
            </div>
